...
def get_all_mask_points(masks_dir):
    ...
    for path in mask_paths:
        ...
        xs = mask_points[1]
        ys = mask_points[0]
        mask_points_all.append(
            np.stack((xs, ys, np.ones_like(xs))).astype(np.float))
...
def refine_visual_hull(masks, Ps, scale, center):
    ...
    GRID_SIZE = 100
    # Fitted for DTU, might need to change for different data.
    MINIMAL_VIEWS = 45
    ...
    for i in range(num_cam):
        ...
        relevant_inds = np.logical_and(
            proj_pixels[0] >= 0, proj_pixels[1] < im_height)
        relevant_inds = np.logical_and(
            relevant_inds, proj_pixels[0] < im_width)
        ...
        relmask = cur_mask[proj_pixels[1, relevant_inds],
                           proj_pixels[0, relevant_inds]]
...
def get_normalization_function(Ps, mask_points_all, number_of_normalization_points, number_of_cameras, masks_all):
    ...
    for i in indss:
        ...
        if observerved_in_all:
            direction = np.linalg.inv(
                P_0[:3, :3]) @ np.array([curx, cury, 1.0])
    ...
    centroid, scale, all_Xs = refine_visual_hull(
        masks_all, Ps, scale, centroid)
...
def get_normalization(source_dir, use_linear_init=False):
    ...
    for i in range(number_of_cameras):
        ...
        cameras_new['world_mat_%d' % i] = np.concatenate(
            (Ps[i], np.array([[0, 0, 0, 1.0]])), axis=0).astype(np.float32)

    np.savez('{0}/{1}_new.npz'.format(source_dir,
             cameras_filename), **cameras_new)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--source_dir', type=str, default='',
                        help='data source folder for preprocess')
    parser.add_argument('--dtu', default=False, action="store_true",
